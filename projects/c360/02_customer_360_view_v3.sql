-- ============================================================
-- ANALYTICS.CUSTOMER_360 — VIEW v3
-- Project: PROJ-C360
-- Owner: Martin Alvarez
-- Version: v3 — 2026-06-13
-- BR006 revision — 2026-06-10: STATUS != 'open' → STATUS = 'resolved'
-- BR011 addition — 2026-06-13: HIGH_VALUE_FLAG concentration risk signal
--
-- Changes from v2:
--   BR011: portfolio_total CTE (CTE 7) — scalar portfolio denominator
--          high_value_threshold CTE (CTE 8) — named 15% constant
--          HIGH_VALUE_FLAG added to Group 6: Risk Indicators (now 4 fields)
--          View expands from 38 → 39 columns
--          Approved: Martin Alvarez, 2026-06-13
--
-- Business rules implemented: BR001-BR008, BR011, A001
...
-- BR006: AVG_SATISFACTION_SCORE on resolved tickets only.
...
-- ============================================================
-- CTE 7: portfolio_total
-- Scalar aggregate: total delivered revenue across all customers.
-- BR011: denominator for HIGH_VALUE_FLAG concentration calculation.
-- References order_metrics — one row output.
-- ============================================================
portfolio_total AS (
    SELECT SUM(TOTAL_ORDER_VALUE_USD) AS TOTAL_PORTFOLIO_VALUE_USD
    FROM order_metrics
),

-- ============================================================
-- CTE 8: high_value_threshold
-- Named constant for BR011 concentration threshold.
-- Single change point if threshold is revised in future.
-- ============================================================
high_value_threshold AS (
    SELECT 0.15 AS THRESHOLD_PCT  -- BR011: >15% of portfolio = high value
)
...
-- BR011: CROSS JOINs bring in portfolio scalar and threshold constant.
-- ============================================================
SELECT
    ...
    -- GROUP 6: Risk Indicators (4 fields)
    ...
    CAST(
        CASE WHEN COALESCE(om.TOTAL_ORDER_VALUE_USD, 0)
                  > pt.TOTAL_PORTFOLIO_VALUE_USD * hvt.THRESHOLD_PCT
             THEN 1 ELSE 0 END
    AS BOOLEAN)                              AS HIGH_VALUE_FLAG
    -- BR011: TRUE if customer > 15% of total portfolio value
...
CROSS JOIN portfolio_total     pt
    -- BR011: portfolio denominator — scalar, one row

CROSS JOIN high_value_threshold hvt
    -- BR011: threshold constant — scalar, one row
...
-- V4: BR011 check — HIGH_VALUE_FLAG concentration results
SELECT
    CUSTOMER_ID,
    COMPANY,
    ...
    ROUND(
        TOTAL_ORDER_VALUE_USD
        / (SELECT SUM(TOTAL_ORDER_VALUE_USD) FROM ANALYTICS.CUSTOMER_360)
        * 100,
    1) AS PORTFOLIO_PCT,
    HIGH_VALUE_FLAG
FROM ANALYTICS.CUSTOMER_360
