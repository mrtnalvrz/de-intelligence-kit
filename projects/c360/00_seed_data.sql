...
-- FILE: 00_seed_data.sql
-- PURPOSE: Self-contained seed — no S3, no external stage needed.
--          All data is embedded as INSERT statements.
--          Works on: Snowflake · Redshift · Databricks · DuckDB · PostgreSQL
-- ============================================================
-- PLATFORM NOTES:
--   Snowflake  → Run as-is. Use ACCOUNTADMIN or a role with CREATE DATABASE.
--   Redshift   → Replace CREATE DATABASE with your existing DB context.
--                Replace TIMESTAMP_NTZ with TIMESTAMP.
--   Databricks → Replace CREATE DATABASE with CREATE SCHEMA.
--                Replace TIMESTAMP_NTZ with TIMESTAMP.
--   DuckDB     → Run as-is (great for local testing, zero infrastructure).
--   PostgreSQL → Replace NUMBER with NUMERIC. Remove TIMESTAMP_NTZ → TIMESTAMP.
-- ============================================================

-- ── STEP 1: DATABASE / SCHEMA SETUP ─────────────────────────
-- Snowflake / DuckDB syntax — adjust for your platform
...
-- The three tables intentionally use different column names
-- for the customer identifier — this is the harmonization
-- challenge the Modern Prometheus DE Intelligence will solve.

-- Table 1: CRM — key field: CUSTOMER_ID
...
-- Table 2: ERP — key field: CUST_ID (intentionally different)
...
-- Table 3: Support — key field: CUSTOMER_REF (third different name)
...
-- ── STEP 3: SEED DATA ────────────────────────────────────────
-- 15 CRM customers | 30 ERP orders | 25 support tickets
-- Data quality issues embedded by design (for the agent to find):
--   • Mismatched join keys across all three tables
--   • Two open tickets with NULL resolved_at (TKT-5007, TKT-5020)
--   • One churned customer with a cancelled order (C007)
--   • One at-risk customer (C012 ManufaCT, CSAT=2)
--   • One high-value customer with open critical security ticket (C013 BankWest)
--   • Two international customers with Brazilian phone format (C009, C015)
...
-- ── STEP 5: CREATE ANALYTICS SCHEMA (target) ─────────────────
CREATE SCHEMA IF NOT EXISTS ANALYTICS;

SELECT 'Seed complete. RAW_DATA and ANALYTICS schemas ready.' AS status;
